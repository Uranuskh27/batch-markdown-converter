...
from the official Release and verify it against the included `SHA256SUMS.txt`. If macOS blocks the
first launch, Control-click the app in Finder and choose **Open**.
...
attach only the DMG and checksum from `release/English/`, plus every source archive and checksum from
`release/Source-Code/`, to the GitHub Release. See `DISTRIBUTION.md` for Developer ID signing and
Apple notarization instructions for public releases.
