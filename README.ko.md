...
- 변환 Worker 프로세스 격리
- 폴더 선택 버튼 하나로 출력 위치를 지정하고 실제 저장 경로를 표시
...
- [Microsoft MarkItDown 0.1.6](https://github.com/microsoft/markitdown) — 수정하지 않은 문서
  변환 엔진, MIT License.
- [Qt for Python / PySide6 및 Shiboken6 6.11.1](https://doc.qt.io/qtforpython-6/) — 수정하지
  않은 GUI 런타임, LGPLv3.
- [Python 3.12](https://docs.python.org/3.12/license.html) — 앱에 포함된 인터프리터 런타임,
  Python Software Foundation License Version 2.
- [PyInstaller 6.21.0](https://github.com/pyinstaller/pyinstaller) — 앱 패키징 도구와 포함된
  bootloader, PyInstaller Bootloader Exception이 적용된 GPL-2.0-or-later. 포함된 run-time
  hook은 Apache-2.0.

DMG의 `Third-Party-Licenses/COMPONENTS.md`에는 배포되는 모든 구성요소의 버전, 선언된
라이선스, 원본 프로젝트 링크와 라이선스 파일이 들어 있습니다. 달리는 강아지·뼈다귀·해·달은
이 프로젝트를 위해 직접 그린 QPainter 벡터이며 제3자 이미지나 폰트 파일을 사용하지 않습니다.
